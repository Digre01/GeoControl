...
describe('GET /networks/{networkCode}/gateways/{gatewayMac}/sensors/{sensorMac}/outliers (e2e)', () => {
    ...
    it('outliers for specific sensor without date filters', async () => {
        const response = await request(app)
            .get(`/api/v1/networks/${TEST_NETWORKS.network_1.code}/gateways/${TEST_GATEWAYS.gate1_net1.macAddress}/sensors/${TEST_SENSORS.sensor1_gate1_net1.macAddress}/outliers`)
    ...
    it('network not found - 404', async () => {
        const response = await request(app)
            .get(`/api/v1/networks/NONEXISTENT/gateways/${TEST_GATEWAYS.gate1_net1.macAddress}/sensors/${TEST_SENSORS.sensor1_gate1_net1.macAddress}/outliers`)
            .set('Authorization', `Bearer ${token}`)
            .expect(404);
    ...
    it('gateway not found - 404', async () => {
        const response = await request(app)
            .get(`/api/v1/networks/${TEST_NETWORKS.network_1.code}/gateways/FF:FF:FF:FF:FF:FF/sensors/${TEST_SENSORS.sensor1_gate1_net1.macAddress}/outliers`)
            .set('Authorization', `Bearer ${token}`)
            .expect(404);
    ...
    it('sensor not found - 404', async () => {
        const response = await request(app)
            .get(`/api/v1/networks/${TEST_NETWORKS.network_1.code}/gateways/${TEST_GATEWAYS.gate1_net1.macAddress}/sensors/FF:FF:FF:FF:FF:FF/outliers`)
            .set('Authorization', `Bearer ${token}`)
            .expect(404);
    ...
    it('correct outlier detection', async () => {
        const response = await request(app)
            .get(`/api/v1/networks/${TEST_NETWORKS.network_2.code}/gateways/${TEST_GATEWAYS.gate1_net2.macAddress}/sensors/${TEST_SENSORS.sensor1_gate1_net2.macAddress}/outliers`)
            .set('Authorization', `Bearer ${token}`)
            .expect(200);
