...
export async function updateNetwork(codeToUpdate: string, networkDto: NetworkDTO): Promise<void> {
  const networkRepo = new NetworkRepository();
  await networkRepo.updateNetwork(codeToUpdate, networkDto.code, networkDto.name, networkDto.description);
}